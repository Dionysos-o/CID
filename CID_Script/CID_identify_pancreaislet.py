...
def apply_mask_pattern_for_pairs(token_ids, MASK_TOKEN_ID):
    # read gene pos dict from a pt file(you need to generate this file by using the potential driver genes and target genes name)
    loaded_gene_positions = torch.load('../data/gene_positions.pt')
...
input_data = sc.read_h5ad('../data/Muraro_pre.h5ad')

dist_mat_vol = []
flag_mat_vol = []
prob_mat_vol = []

path = './panglao_pretrain.pth'
...
for index in range(1, input_data.shape[0]):
    cell_type_column = 'label'
    cell_type = input_data.obs[cell_type_column].iloc[index]
    # change the cell type here
    if cell_type in ['alpha']:
        ...
        print(dist_matrix)
        dist_mat_vol.append((dist_matrix,index,cell_type))


torch.save(dist_mat_vol, 'dist_mat_vol_alpha_test.pt')
